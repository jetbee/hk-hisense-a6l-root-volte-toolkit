...
かなりのパッチ作業を経た後、実際に楽天モバイルSIM（MCC/MNC `440-11`）でVoLTEと一般データの両方を解決したのは、拍子抜けするほどシンプルな方法でした: **無改造の別のキャリアプロファイルをそのままActivateして様子を見る。** これは自分で思いついたものではなく、そのまま[XDA: \[Guide\] Enabling VoLTE/VoWiFi (deprecated)](https://xdaforums.com/rog-phone-2/how-to/guide-enabling-volte-vowifi-t4023529)が元ネタです（対象機種は全く別のROG Phone 2、Snapdragon 855向け）。あのスレッドの手法まるごとが「プロファイル集を片っ端から試して、どのキャリアプロファイルで誰のVoLTEが有効になるか記録する」というものです——MCFGキャリアポリシーは特定のオペレーターではなく**国（MCC）単位**でスコープされていることが多く、自分とは無関係な別キャリアのプロファイルが自分の国向けVoLTEを解除してしまうことが頻繁にあります。このテクニックのアイデアは全部あのスレッドのもので、クレジットはこちらではなくあちらに帰属します。
